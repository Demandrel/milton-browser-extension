...
<step n="10.5" goal="Create Mandatory Documentation Story">

<critical>Documentation is NOT optional. Every epic that ships features MUST have a documentation story created here. The story is tracked in sprint-status and blocks the next epic from starting cleanly. This step creates the story file — it does NOT write the docs inline.</critical>

<output>
Bob (Scrum Master): "Before we save — documentation. This is mandatory. Every epic gets a documentation story, no exceptions."

Bob (Scrum Master): "Epic {{epic_number}} shipped new functionality. We need a tracked, reviewable documentation story — not an action item that gets forgotten."
</output>

<action>Identify which documentation pages need updating based on Epic {{epic_number}}:</action>

**Feature Documentation:**
<action>Check if `docs/features/` has a page corresponding to Epic {{epic_number}} using this mapping:
  - Epic 1 → `docs/features/authentication.mdx`
  - Epic 2 → `docs/features/reference-library.mdx`
  - Epic 3 → `docs/features/collections-projects.mdx`
  - Epic 4 → `docs/features/reading-notes.mdx`
  - Epic 5 → `docs/features/search-navigation.mdx`
  - Epic 6 → `docs/features/settings.mdx`
  - Epic 7 → `docs/features/freemium.mdx`
  - Epic 8 → no feature page (documentation epic)
</action>
<action>If a matching page exists, note it needs updating with consolidated story notes from all completed stories</action>
<action>If no matching feature page exists (e.g. Epic 0, Epic 8), skip to the check below</action>

**Architecture Documentation:**
<action>Review story Dev Notes across all Epic {{epic_number}} stories for new architectural patterns</action>
<action>If new patterns found, note `docs/architecture/overview.mdx` needs updating</action>

**Developer Guide Documentation:**
<action>Review story Dev Notes for new commands, conventions, tools, or patterns</action>
<action>If new developer-facing changes found, note which `docs/developer-guide/` pages need updating</action>

**Freshness Check:**
<action>Scan existing doc pages for content that may have become stale due to Epic {{epic_number}} changes (refactoring, dependency upgrades, renamed files, etc.)</action>

<check if="no documentation pages need updating (e.g. Epic 0, Epic 8, or purely technical epic with no user-facing changes)">
  <output>
Bob (Scrum Master): "Epic {{epic_number}} has no documentation pages to update — skipping documentation story creation."
  </output>
  <action>Set {{doc_story_created}} = false</action>
  <action>Skip to end of step</action>
</check>

<action>MANDATORY: Create a documentation story file at {story_directory}/{{epic_number}}-docs-update-documentation.md</action>

<action>The story file MUST contain the following structure:</action>

```markdown
# Story {{epic_number}}-docs: Update Documentation After Epic {{epic_number}}
...
- **Created From:** Epic {{epic_number}} retrospective (auto-generated by Step 10.5)

## Description

Update all documentation pages affected by Epic {{epic_number}}: {{epic_title}}.

This story was auto-generated by the retrospective workflow. It MUST be completed before
the next epic begins. Documentation is not optional — it is a tracked deliverable.

## Acceptance Criteria

### AC1: Feature Documentation Updated
- [ ] {{feature_page_path}} is updated with consolidated implementation details from all Epic {{epic_number}} stories
- [ ] All placeholder content is replaced with real documentation
- [ ] Code examples reflect actual implementation (not hypothetical)
- [ ] File paths and component names are verified against the codebase

### AC2: Architecture Documentation Updated (if applicable)
{{#if architecture_updates_needed}}
- [ ] `docs/architecture/overview.mdx` updated with new patterns: {{list_new_patterns}}
{{else}}
- [x] No architecture updates needed for Epic {{epic_number}}
{{/if}}

### AC3: Developer Guide Updated (if applicable)
{{#if devguide_updates_needed}}
- [ ] {{list_devguide_pages}} updated with: {{list_devguide_changes}}
{{else}}
- [x] No developer guide updates needed for Epic {{epic_number}}
{{/if}}

### AC4: Freshness Verified
- [ ] Existing doc pages scanned for stale content caused by Epic {{epic_number}} changes
- [ ] Any stale references updated (renamed files, changed APIs, dependency upgrades)

## Documentation Pages to Update

{{list_pages_needing_updates_with_details}}

## Pages Confirmed Current (No Changes Needed)

{{list_pages_still_fresh}}

## Source Material

- Story files: {story_directory}/{{epic_number}}-*.md (read Dev Notes, Implementation Notes, and acceptance criteria)
- Retrospective: {retrospectives_folder}/epic-{{epic_number}}-retro-{date}.md

## Commit Convention

`docs: update documentation after Epic {{epic_number}}`
```

<action>Save the story file to {story_directory}/{{epic_number}}-docs-update-documentation.md</action>

<action>MANDATORY: Register the story in {sprint_status_file}:</action>
<action>Add key "{{epic_number}}-docs-update-documentation" with status "draft" under development_status</action>
<action>Save {sprint_status_file} preserving ALL comments and structure</action>

<output>
Bob (Scrum Master): "Documentation story created and registered:"

**Documentation Story for Epic {{epic_number}}:**

- Story file: `{story_directory}/{{epic_number}}-docs-update-documentation.md`
- Sprint status: `{{epic_number}}-docs-update-documentation: draft`
- Assigned to: Paige (Tech Writer)
- Pages to update:
{{list_pages_needing_updates}}

Bob (Scrum Master): "This story goes through the normal workflow: create-story to mark ready → dev-story to implement → code-review to verify. No shortcuts."

Bob (Scrum Master): "It should be completed before starting Epic {{next_epic_num}}. It's tracked in sprint-status just like any other story."

Bob (Scrum Master): "{user_name}, anything else documentation-wise we should capture in this story?"
</output>

<action>WAIT for {user_name} to confirm or add documentation notes</action>

<action>If {user_name} adds notes, update the story file with additional requirements</action>

<action>Set {{doc_story_created}} = true</action>
<action>Store {{doc_update_plan}} for inclusion in retrospective summary document</action>
...
- Documentation story reference (story file path, sprint-status key, pages to update) — or note that no doc story was needed
...
{{#if doc_story_created}}
2. **Complete documentation story** (`{{epic_number}}-docs-update-documentation`)
   - Run `create-story` to mark ready → `dev-story` to implement → `code-review` to verify
   - Must be completed BEFORE starting Epic {{next_epic_num}}
   - Tracked in sprint-status like any other story
{{/if}}

{{#if doc_story_created}}3{{else}}2{{/if}}. **Execute preparation sprint** (Est: {{prep_days}} days)
   - Complete {{critical_count}} critical path items
   - Execute {{prep_task_count}} preparation tasks
   - Verify all action items are in progress

{{#if doc_story_created}}4{{else}}3{{/if}}. **Review action items in next standup**
   - Ensure ownership is clear
   - Track progress on commitments
   - Adjust timelines if needed

{{#if epic_update_needed}}
{{#if doc_story_created}}5{{else}}4{{/if}}. **IMPORTANT: Schedule Epic {{next_epic_num}} planning review session**
...
{{else}}
{{#if doc_story_created}}5{{else}}4{{/if}}. **Begin Epic {{next_epic_num}} when ready**
   - Start creating stories with SM agent's `create-story`
   - Epic will be marked as `in-progress` automatically when first story is created
   - Ensure all critical path items are done first
   {{#if doc_story_created}}- Ensure documentation story is DONE first{{/if}}
{{/if}}
...
{{#if doc_story_created}}
⚠️ **REMINDER**: Documentation story `{{epic_number}}-docs-update-documentation` must be completed before starting Epic {{next_epic_num}}
{{/if}}
