...
- [x] **Task 8 — Automated checks GREEN; sideload smoke awaits Pierre** (AC: #15, #16)
  ...
  - [x] 8.3 `pnpm build` — succeeds (684ms). Bundle size delta: +2.3 kB gz in the popup chunk (well under the 30 kB heuristic budget). Other chunks unchanged.
  - [ ] 8.4 Sideload smoke S1-S7 per AC15 — **awaits Pierre's manual run** post-PR (BE-8-6 pattern: smoke runs after the PR opens; Pierre pastes traces into this file before code-review).
  - [ ] 8.5 Verify against actual Milton library: PDFs attached in S1/S2/S4 (success message shows the dual-tone icon); reference saved without PDF in S5; reference saved with `pdfUrl` in S3 (icon ABSENT — `pdfAttached` is undefined for `'be-7-fallback'` mode); regression check on a public-host PDF (S1).
