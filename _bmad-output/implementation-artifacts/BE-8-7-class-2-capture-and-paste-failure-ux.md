...
From `/bmad_bmm_code-review` (2026-05-18). HIGH/MED/LOW fixed in code where possible (see Change Log entry for the fix commit); these are the residual action items that need sideload verification or external (Figma) input.

- [ ] **[AI-Review][MED] M1 — verify CORS preflight on `POST /references/{id}/pdf-bytes` against a non-arxiv host.** AC15 S1 explicitly requires opening DevTools Network panel and confirming an `OPTIONS` preflight precedes the POST with `Access-Control-Allow-Headers: Content-Type` + `Access-Control-Allow-Methods: POST`. Original Task 8.4 entry deferred this and inferred from round-trip success. Inference doesn't catch a 200-on-arxiv-only-because-it-tolerates-anything scenario; preflight may quietly fail on stricter publisher hosts. **Action:** sideload `dist/` in Chrome, open Network panel, save a PDF on a non-arxiv host, confirm `OPTIONS` 204/200 with correct headers. If absent → file as BE-8-2 follow-up in Milton-saas.
- [ ] **[AI-Review][MED] M2 — run S3 (Flow A → BE-7 fallback) and S7 (tab-staleness) smokes.** Both are ~30-second sideload tests, not "low marginal value":
  - **S3:** clear cookies for a publisher domain in DevTools → click toolbar on its direct-PDF URL → expect `[milton-popup] pdf-class2-fallback reason=<code>` log + reference saved via BE-7's `pdfUrl` path. Verifies the fallback regression-check the spec called out.
  - **S7:** pin the popup open via "Inspect popup" → navigate the underlying tab to a Class 2 PDF → click toolbar → close the underlying tab during the in-tab fetch. Expect `TAB_GONE` → graceful soft-degrade, no hung popup.
- [ ] **[AI-Review][HIGH-follow-up] H2 — consult Figma for the success-with-PDF treatment.** `popup.ts:PDF_ICON_SVG` ships a provisional inline dual-tone SVG. Per CLAUDE.md Rule 1 (FIGMA VERIFICATION FOR ALL UI WORK), this should be Figma-derived. **Action:** Pierre connects Figma MCP → review node `1323:8984` for any "Saved with PDF" treatment → if a design exists, replace `PDF_ICON_SVG` (and the `.milton-popup-pdf-icon` CSS) verbatim; if no design exists, mark the provisional as Figma-blessed in the story. Code TODO marker at `src/popup/popup.ts:PDF_ICON_SVG` for grep recovery.
...
| Date | Author | Note |
|---|---|---|
| 2026-05-18 | Claude (Opus 4.7 1M, `/bmad_bmm_code-review`) | **Adversarial code-review on the open PR. Found 3 HIGH / 6 MEDIUM / 4 LOW; fixed all in code where possible.** HIGH: H1 (AC12a / BT6 — 409 re-encounter PDF affordance) wired into `error-409-duplicate` render + `wasPdfContext` captured at dispatch time; H2 (Figma rule violation on `PDF_ICON_SVG`) marked with TODO + filed as follow-up; H3 (~100 LOC dead XHR branch in `attachPdfBytes`) deleted, 6 XHR tests removed. MED: M3 (duplicated cast → extracted local), M4 (Flow B `pendingPdfAttachmentUrl` nulled symmetrically with Flow A bytes), M5 (`dispatchCreateReferenceResult` signature narrowed to error subset, dead success branch removed), M6 (external-cancel vs local-timeout now surface different statuses — `network-error` vs `timeout`). LOW: L3 (1500 promoted to `SUCCESS_AUTO_CLOSE_MS`), L4 (comment imprecision fixed), L1 (BT7-untestability noted at the in-tab function). Residual items requiring sideload (M1 CORS preflight + M2 S3/S7 smokes) and Figma input (H2 follow-up) filed in `Review Follow-ups (AI)` subsection. Verified: typecheck clean, tests **368/368 pass** (was 372; -6 XHR tests + 2 new M6 cancel tests = net -4), build succeeds. |
