...
6. **Popup state machine extended.** Three new top-level states (per Charter v2 story-map):
   - `translator-running { url, html, candidateIds[], chosenId }` — shown while the client translator executes. Renders a loading panel with the publisher name (derived from the chosen translator's `metadata.label`) and a cancel button. Default render delay: 200ms (avoids flicker if translation completes faster than perceptible).
...
15. **Smoke matrix (Pierre G17-1) — six scenarios; ALL PASS required before review.** Sideload `dist/` into Chrome via `chrome://extensions/`. For each scenario, capture console output as evidence in Completion Notes.
...
- [x] **Task 9 — Smoke + sideload verification** (AC: #15, #17, #18)
  ...
  - [~] 9.6 Sideload smoke S1-S6 — **PENDING Pierre's manual run**. The story file lists the six scenarios; dev-agent can't execute manual Chrome sideload from this session. Pierre to run + paste console traces into Completion Notes before flipping to `done`.
  - [~] 9.7 Verify offscreen document creation — **PENDING Pierre's manual check** (chrome://extensions/ → Inspect views → confirm `offscreen.html` document listed during a translation).
...
- [x] **Task 10 — Pre-Review Self-Check + PR** (AC: #18)
  ...
  - [ ] 10.4 `git push` + `gh pr create` (in progress this turn).
...
- `pnpm test`: 303/303 pass (220 BE-8-5 baseline + 83 new BE-8-6 tests across page-context, translator-router, offscreen, offscreen-client, zotero-item-to-payload, popup-helpers extensions)
- `pnpm build`: success — sandbox+translator-bundle chunks combined ≈441.7 kB gz (essentially unchanged from BE-8-5 baseline 442 kB); new offscreen chunk 1.47 kB gz; popup chunk includes page-context/router/item-mapper tree-shaken (14.95 kB gz index.html-* chunk)
...
- `src/popup/popup-helpers.ts` — Extracted shared `isRestrictedUrl(url)` helper; `detectPdfPage` refactored to reuse it (single restricted-URL list, no drift)
- `src/popup/popup-helpers.test.ts` — Added 12 new tests: 9 `isRestrictedUrl` branches + 3 `detectPdfPage × isRestrictedUrl` integration tests
- `src/translator-runtime/translator-bundle.ts` — Added `listBundledTranslators()` export (outside GENERATED block — refresh-script idempotency preserved)
- `src/translator-runtime/sandbox.ts` — Added `eagerRegisterBundled(verifiedSet)` in `bootstrapAll`; doc comment records the decision + lazy-fallback escape hatch
...
- `src/translator-runtime/translator-fetcher.ts` — BE-8-5 CDN-fetch unchanged
