...
- [ ] **Task 4 — Add AGPL `COPYING` + per-file SPDX headers in the new repo** (AC: #3)
  - [ ] 4.1 [D] Clone the new repo fresh (not the scratch one — the throwaway has the extracted history pushed; further work lands via PRs against the new repo's `main`).
  - [ ] 4.2 [D] On a feature branch (`license/agpl-3.0`): write `COPYING` with the full GNU AGPL v3 text (curl `https://www.gnu.org/licenses/agpl-3.0.txt` for the canonical source; verify SHA-256 against gnu.org).
  - [ ] 4.3 [D] Update `package.json` `license` field to `"AGPL-3.0-or-later"` (BE-1 likely set it to `"UNLICENSED"` or omitted it).
  - [ ] 4.4 [D] Write `scripts/add-spdx-headers.sh`: idempotent shell script that prepends the SPDX header (3 lines, comment style by file extension) to all `src/**` files matching `.ts | .html | .css`. Idempotency: skip files whose first 200 bytes already contain `SPDX-License-Identifier`.
  - [ ] 4.5 [D] Run the script; commit with message `chore(license): add SPDX headers + COPYING (AGPL-3.0-or-later)`.
  - [ ] 4.6 [D] Open PR against new repo's `main`; this is the first ordinary PR. Pre-push hook (if set up — see Task 6) runs typecheck + test + build.
