...
4. **Store-listing assets prepared in `store-assets/cws/`.**
    ...
    - `screenshots/` — 3-5 PNG files at 1280×800 (CWS preferred) or 640×400 (also accepted). Required coverage: (i) **popup metadata preview on arXiv** (happy path, Class 3 capture); (ii) **tags + projects + collections selectors** (BE-2 UX showcase); (iii) **signed-out state** with "Open Milton" CTA (proves the auth handling story); (iv) **translator-fallback state** on a bot-protected publisher (shows the honest beta caveat in UI form). Fifth slot optional.
...
- [x] **Task 4 — Store-listing copy + assets** (AC: #4, #9)
  ...
  - [x] 4.4 Captured 3 screenshots (1280×800 PNG) in `store-assets/cws/screenshots/` — `Screen BE 1/2/3.png`. Meets AC4 "3-5 files" minimum.
...
- **Sandbox-page CSP `'unsafe-eval'` is intentional + load-bearing.** zotero/translate framework runs translator JS via `new Function()` / eval. We declare `sandbox.pages` in manifest.config.ts:69-72 specifically to permit this. Comment in manifest.config.ts is already there; permissions.md needs to reference it.
...
- [Source: manifest.config.ts:69-72] — sandbox.pages declaration enabling 'unsafe-eval' in the translator runtime context.
...
| 2026-05-21 | Pierre + Claude (Opus 4.7 1M, BMad Master operational guidance) | Release sequencing Phase A. 3rd screenshot captured (`Screen BE 3.png`) — 3×1280×800 now committed, meets AC4 minimum. `PRIVACY.md` support email finalized as `support@milton.so`. Story checkboxes synced: Task 4 done; Task 4.6 marquee skipped (optional). Next: open BE-8-10 PR → CI → `/bmad_bmm_code-review` → merge to `main`, then operational Phases B-E (GitHub Pages enablement, CWS account registration, fresh-profile smoke, submission). |
