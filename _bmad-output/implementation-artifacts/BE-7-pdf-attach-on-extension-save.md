...
- [ ] Task 7 (AC: 7) — Manual sideload smoke (Pierre runs at code review time)
  - [ ] Scenarios 1 + 2 + 3 from AC7 — happy path across PDF URL / arXiv PDF / arXiv abs page (covers the new direct path AND the now-wired OA path).
  - [ ] Scenarios 5 + 7 — `.pdf.html` rejection + paywall-HTML magic-byte rejection.
  - [ ] Scenario 9 — 409 dedup-is-noop verified (existing PDF not replaced).
  - [ ] Scenario 12 — cross-content cycle (PDF → article → PDF) to confirm no state leak.
...
- [ ] Manual sideload smoke (Task 7) — Pierre's gate at review time per G17-1
